...
def classifyDfrc(whitepieces, blackpieces):
    ...
    if whitepieces == blackpieces:
        return "FRC " + classifyFrc(whitepieces)
    else:
        return "DFRC " + classifyFrc(whitepieces) + ":" + classifyFrc(blackpieces)

def classifyFrcFromStdin():
    openingTags = False
    frcClass = None

    reOpeningTags = re.compile(r'\[(Opening|Variation).*\]')
    reFEN = re.compile(r'\[FEN "([nbrqk]{8})/pppppppp/8/8/8/8/PPPPPPPP/([NBRQK]{8}) w (K?Q?k?q?|-) - 0 1"\]')
    ...
    for line in sys.stdin:
        ...
        if line:
            print(line)
            if (reOpeningTags.fullmatch(line)):
                openingTags = True
